...
for (let i = 0; i < targetShape.length; i++) {
  const pointsArr = targetShape[i].points.split(' ').map((point) => {
...
export default function InteractivePuzzle({
  ...
  const [targetPieces] = useState(targetShape)
  ...
  const enhancedHandleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
